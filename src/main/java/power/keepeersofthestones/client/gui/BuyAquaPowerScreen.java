...
public class BuyAquaPowerScreen extends AbstractContainerScreen<BuyAquaPowerMenu> {
	...
	public BuyAquaPowerScreen(BuyAquaPowerMenu container, Inventory inventory, Component text) {
	...
	private static final ResourceLocation texture = new ResourceLocation("power:textures/buy_aqua_power.png" );
	...
	@Override protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY) {
		    	this.font.draw(poseStack, "To buy, put the required number of coins in the slot (12)",
					46, 7, -12829636);
	}
				...
				this.addRenderableWidget(new Button(this.leftPos + 217, this.topPos + 34,
					40, 20, new TextComponent("Buy"), e -> {
							if (        true
								...
								PowerMod.PACKET_HANDLER.sendToServer(new BuyAquaPowerButtonMessage(0, x, y, z));
								BuyAquaPowerButtonMessage.handleButtonAction(entity, 0, x, y, z);
							}
					}
				)
);
