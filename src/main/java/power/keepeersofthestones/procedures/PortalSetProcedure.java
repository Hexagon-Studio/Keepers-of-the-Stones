	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (PowerModVariables.MapVariables.get(world).orange_portal) {
			if (!PowerModVariables.MapVariables.get(world).blue_portal) {
				world.setBlock(new BlockPos((int) x, (int) (y + 1), (int) z), PowerModBlocks.BLUE_PORTAL.defaultBlockState(), 3);
			...
			if (!PowerModVariables.MapVariables.get(world).orange_portal) {
				world.setBlock(new BlockPos((int) x, (int) (y + 1), (int) z), PowerModBlocks.ORANGE_PORTAL.defaultBlockState(), 3);
